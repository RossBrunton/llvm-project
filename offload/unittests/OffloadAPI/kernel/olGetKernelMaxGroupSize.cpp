//===------- Offload API tests - Kernel->maxGroupSize ------------------===//
//
...
TEST_P(olKernelGetMaxGroupSizeTest, Success) {
  size_t Size{0};
  ASSERT_SUCCESS(Kernel->maxGroupSize(Device, Kernel, 0, &Size));
  ASSERT_GT(Size, 0u);
}
...
TEST_P(olKernelGetMaxGroupSizeTest, NullKernel) {
  ...
  ASSERT_ERROR(OL_ERRC_INVALID_NULL_HANDLE,
               Kernel->maxGroupSize(Device, nullptr, 0, &Size));
}

TEST_P(olKernelGetMaxGroupSizeTest, NullDevice) {
  size_t Size;
  ASSERT_ERROR(OL_ERRC_INVALID_NULL_HANDLE,
               Kernel->maxGroupSize(nullptr, Kernel, 0, &Size));
}

TEST_P(olKernelGetMaxGroupSizeTest, NullOutput) {
  ASSERT_ERROR(OL_ERRC_INVALID_NULL_POINTER,
               Kernel->maxGroupSize(Device, Kernel, 0, nullptr));
}
